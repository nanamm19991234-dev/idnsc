...
function SpeedFactor(distance, speed, multiplier)
    local baseSpeed = speed or (Settings and Settings["Tween Speed"]) or 200
    local factor = multiplier or _G.SpeedFactor or getgenv().SpeedFactor or 1
    if baseSpeed <= 0 then baseSpeed = 200 end
    if factor <= 0 then factor = 1 end
    return (distance / baseSpeed) / factor
end
...
_tp = function(target)
    ...
    local tweenDuration = SpeedFactor(distance, Settings["Tween Speed"])
...
-- Icon Configuration:
-- Put your rbxassetid here once uploaded to Roblox (e.g., "rbxassetid://123456789"),
-- or leave as "" to use local 'idn logo.png' via getcustomasset.
local CustomIconID = "" 
local geticon = (getcustomasset or getsynasset or function(a) return a end)
local idnIcon = (CustomIconID ~= "" and CustomIconID) or geticon("idn logo.png")

local redzlib = (readfile and isfile and (isfile("ui.lua") or isfile("ui.luau")) and loadstring(readfile(isfile("ui.lua") and "ui.lua" or "ui.luau"))()) or loadstring(game:HttpGet("https://raw.githubusercontent.com/bloxfruitsnokey/Redz/refs/heads/main/Emerald/ui.luau"))()

local Window = redzlib:MakeWindow({
    Title = "idnsc : Blox Fruit",
    SubTitle = "by @3.1 boy 26/27",
    Icon = idnIcon,
    SaveFolder = "idnsc.json"
})
...
local MobileButton = Minimizer:CreateMobileMinimizer({
  Image = idnIcon,
  BackgroundColor3 = Color3.fromRGB(15, 35, 65)
})

-- Square shape with rounded corners for the icon button + White border
pcall(function()
    if MobileButton then
        local corner = MobileButton:FindFirstChildOfClass("UICorner") or Instance.new("UICorner")
        corner.CornerRadius = UDim.new(0, 12)
        corner.Parent = MobileButton

        local stroke = MobileButton:FindFirstChildOfClass("UIStroke") or Instance.new("UIStroke")
...
Tabs.Settings:AddSlider({
    ...
    Increment = 0.1,
    Callback = function(Value)
        _G.SpeedFactor = Value
        getgenv().SpeedFactor = Value
    end
})
Tabs.Settings:AddToggle({
